...
class Collection extends \Magento\Catalog\Model\ResourceModel\Collection\AbstractCollection
{
    ...
    {
        // custom changes added for out of stock product display end of the page.
        if (strpos($this->getSelect()->__toString(), 'is_salable')) {
            $this->getSelect()->order("is_salable desc");
        }
        if (strpos($this->getSelect()->__toString(), 'not2order_hide_orderbtn')) {
            $this->getSelect()->order("not2order_hide_orderbtn asc");
        }
        // end
        ...
        if ($attribute == 'position') {
            ...
            if (isset($filters['category_id']) || isset($filters['visibility'])) {
                ...
                $this->getSelect()->order('cat_index.position desc');
            } else {
        ...
        if ($attribute == 'price' && $storeId != 0) {
            ...
            if ($this->_productLimitationFilters->isUsingPriceIndex()) {
                $moqPriceCol = $this->getConnection()->getCheckSql(
                    'price_index.final_price > 0',
                    'min_sale_qty*final_price',
                    '1'
                );
                $colls = [
                    'moq_price' => $moqPriceCol,
                ];
                $this->getSelect()->joinInner(
                    ['ci' => $this->getTable('cataloginventory_stock_item')],
                    'ci.product_id = e.entity_id',$colls)
                     ->order('moq_price '. $dir);
                return $this;
            }
        }
